// ═══════════════════════════════════════════════════════════════
//  ADELE — Background Service Worker
//  Manages WebSocket bridge to the ADELE backend, action
...
function restrictedPageHint(url) {
  const value = String(url || "").trim();
  if (!value) {
    return "Open a normal website tab (https://…) to capture a page snapshot.";
  }
  if (/^chrome-extension:/i.test(value)) {
    return "Extension pages cannot be captured. Switch to a normal https:// website tab.";
  }
  if (/^chrome:\/\//i.test(value)) {
    return "Chrome system pages (like chrome://extensions) cannot be captured. Open a normal https:// tab instead.";
  }
  if (/^edge:\/\//i.test(value) || /^about:/i.test(value)) {
    return "Browser system pages cannot be captured. Open a normal https:// website tab.";
  }
  return "This page cannot be captured. Open a normal https:// or file:// tab.";
}
...
async function executeActionOnTab(action) {
  ...
  try {
    ...
    // ── Scroll action: relay to content script via adele_scroll ──
    ...
    if (action?.action === "extract_readability") {
      const readabilityResult = await chrome.tabs.sendMessage(tab.id, {
        type: "adele_extract_readability",
        sessionId: action?.session_id || sessionId,
        tabId: String(tab.id),
      });
      return buildActionResult(
    ...
    if (result?.ok && isClickLike) {
      chrome.tabs.sendMessage(tab.id, { type: "adele_trigger_click_burst" }).catch(() => {});
    }
...
async function requestSnapshotFromTab(tabId, sessionIdOverride = sessionId) {
  ...
  if (!tab?.id) {
    lastBridgeError = "";
    lastBridgeHint = "No active tab available. Focus a browser tab and try again.";
  ...
  if (!isInjectableUrl(tab.url)) {
    lastBridgeError = "";
    lastBridgeHint = restrictedPageHint(tab.url);
    lastBridgeMessage = "Active tab is not capturable";
...
async function bridgeStatusWithActiveTab() {
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  const activeTabUrl = tab?.url || "";
  return bridgeStatus({
    activeTabUrl,
    activeTabCapturable: isInjectableUrl(activeTabUrl),
    activeTabHint: isInjectableUrl(activeTabUrl) ? "" : restrictedPageHint(activeTabUrl),
  });
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message?.type === "adele_snapshot") {
  ...
  if (message?.type === "adele_dom_change") {
  ...
  if (message?.type === "adele_get_status") {
    bridgeStatusWithActiveTab()
      .then((status) => sendResponse(status))
      .catch(() => sendResponse(bridgeStatus()));
    return true;
  }

  if (message?.type === "adele_request_snapshot") {
    pushActiveTabSnapshot()
      .then(() => bridgeStatusWithActiveTab().then((status) => sendResponse({ ok: true, ...status })))
      .catch((error) => bridgeStatusWithActiveTab().then((status) => sendResponse({
        ok: false,
  ...
  if (message?.type === "adele_reconnect_bridge") {
    ...
    authenticated = false;
    lastBridgeError = "";
    lastBridgeMessage = "Manual reconnect requested";
    loadSettings().then(() => connectBridge());
    sendResponse({ ok: true, ...bridgeStatus() });
    return true;
  }

  if (message?.type === "adele_settings_updated") {
