...
if (window.__adele_injected__) return;
window.__adele_injected__ = true;

// ═══════════════════════════════════════════════════════════════
//  ADELE — Content Script v2
...
async function sendSnapshot(sessionId, tabId) {
  ...
  try {
    ...
    console.debug("[ADELE Content] Snapshot send failed", error);
  }
}
  ...
  const observer = new MutationObserver(function(mutations) {
    ...
    if (_pendingVerify && (Date.now() - _pendingVerify.timestamp) < 10000) {
      try {
        ...
        console.debug("[ADELE Content] DOM change event send failed", e);
      }
...
// ── ADELE Research Highlight Styles ──
const HIGHLIGHT_STYLE_ID = "adele-research-highlight-style";
const RESEARCH_OVERLAY_ID = "adele-research-overlay";
...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
  if (message?.type === "adele_collect_snapshot") {
  ...
  if (message?.type === "adele_evaluate_js") {
    try {
  ...
  if (message?.type === "adele_extract_data") {
    try {
  ...
  if (message?.type === "adele_extract_readability") {
    sendResponse?.(extractReadabilityArticle());
    return true;
  }
  if (message?.type === "adele_execute_action") {
  ...
  if (message?.type === "adele_highlight") {
  ...
  if (message?.type === "adele_scanning_start") {
  ...
  if (message?.type === "adele_scanning_stop") {
    stopScanningMode();
    sendResponse?.({ ok: true });
    return true;
  }
  // ── Agent click pointer ──
  if (message?.type === "adele_show_click_pointer") {
  ...
  if (message?.type === "adele_trigger_click_burst") {
    triggerClickBurst();
    sendResponse?.({ ok: true });
    return true;
  }
