...
ADELE is a **voice-activated desktop agent** that runs as an Electron overlay with a Python backend powered by Gemma 4 model routing. It can control the user's desktop (open apps, click UI, type text) and automate the browser via the Chrome extension bridge.
