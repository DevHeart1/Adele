...
from providers.antigravity import AntigravityProvider, is_antigravity_agent
from providers.gemini import DEFAULT_GEMINI_MODEL, GeminiProvider
...
class Tier(Enum):
    FAST = 1       # Gemini fast tier: routing and simple execution
    POWERFUL = 2   # Gemini powerful tier: complex reasoning and multimodal work
...
# Gemini via Google GenAI. Exact IDs may evolve; override with GEMINI_*_MODEL env vars.
FAST_MODEL = os.environ.get("GEMINI_FAST_MODEL", DEFAULT_GEMINI_MODEL).strip() or DEFAULT_GEMINI_MODEL
POWERFUL_MODEL = os.environ.get("GEMINI_POWERFUL_MODEL", DEFAULT_GEMINI_MODEL).strip() or DEFAULT_GEMINI_MODEL
ROUTING_MODEL = os.environ.get("GEMINI_ROUTING_MODEL", FAST_MODEL).strip() or FAST_MODEL

# The routing prompt that the fast Gemini tier uses to classify requests.
ROUTING_PROMPT = """You are a routing classifier for a desktop AI assistant called ADELE.
...
- **POWERFUL**: Everything else. Gemini powerful tier. Handles all real tasks with full intelligence.
...
class ModelRouter:
    ...
    async def initialize(self):
        ...
        async with self._init_lock:
            ...
            provider_mode = (os.environ.get("ADELE_LLM_PROVIDER") or "gemini").strip().lower()
            ...
            if provider_mode != "gemini" and not (
                os.environ.get("ADELE_LOCAL_MODEL")
                or os.environ.get("ADELE_LOCAL_FAST_MODEL")
            ...
            def get_provider(model_name: str) -> LLMProvider:
                if provider_mode in {"ollama", "local", "openai-compatible-local", "llama.cpp", "llamacpp"}:
                    base_url = (
                        os.environ.get("ADELE_LOCAL_BASE_URL")
                    ...
                    return OpenAICompatibleProvider(
                        base_url=base_url,
                        api_key=os.environ.get("ADELE_LOCAL_API_KEY", "adele-local"),
                        model=model_name,
                        label=label,
                        supports_vision=os.environ.get("ADELE_LOCAL_SUPPORTS_VISION", "0") == "1",
                        supports_tools=os.environ.get("ADELE_LOCAL_SUPPORTS_TOOLS", "1") != "0",
                    )

                if is_antigravity_agent(model_name):
                    return AntigravityProvider(api_key=api_key, agent=model_name)
                return GeminiProvider(api_key=api_key, model=model_name)

            # Routing model (ultra-cheap, ultra-fast tier 0)
            routing_name = (
                os.environ.get("ADELE_LOCAL_ROUTING_MODEL")
                or os.environ.get("ADELE_LOCAL_MODEL")
            ...
            fast_name = (
                os.environ.get("ADELE_LOCAL_FAST_MODEL")
                or os.environ.get("ADELE_LOCAL_MODEL")
            ...
            powerful_name = (
                os.environ.get("ADELE_LOCAL_POWERFUL_MODEL")
                or os.environ.get("ADELE_LOCAL_MODEL")
            ...
            fallback_name = (
                os.environ.get("ADELE_LOCAL_FALLBACK_MODEL")
                or os.environ.get("ADELE_LOCAL_MODEL")
