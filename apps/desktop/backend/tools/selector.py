...
def resolve_milestone_allowed_tools(
    ...
    """Resolve the preferred allowed tools for a milestone.

    If request scope is empty, return None meaning all tools are allowed.
    """
    priority, _fallback = resolve_milestone_tool_tiers(hint_tools, request_scope_tool_names)
    return priority


def resolve_milestone_tool_tiers(
    hint_tools: Iterable[str],
    request_scope_tool_names: Optional[set[str]],
