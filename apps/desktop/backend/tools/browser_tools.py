...
async def browser_refresh_refs(session_id: str = "", timeout: float = 2.0) -> str:
    resolved_session_id = session_id or browser_bridge.connected_session_id() or ""
    snapshot = browser_store.get_snapshot(resolved_session_id or None)
    if not snapshot and not resolved_session_id:
        snapshot = _lookup_snapshot(session_id)
        resolved_session_id = snapshot.session_id if snapshot else ""
    ...
    return json.dumps({
        ...
        "elements": len(snapshot.elements),
        "message": (
            "Browser snapshot bootstrapped from extension."
            if previous_generation <= 1
            else "Using latest known browser snapshot generation."
        ),
        **_snapshot_health(snapshot),
    }, ensure_ascii=False)
