"""
ADELE — Agent-Computer Interface (ACI) Browser Tools
========================================================
...
async def get_page_summary(session_id: str = "") -> str:
    ...
    try:
        readability = await _bridge_extract_readability(session_id=session_id, timeout=4.0)
        readability_text = str(readability.get("text", "") if isinstance(readability, dict) else "").strip()
        readability_length = int(readability.get("content_length", len(readability_text)) or 0) if isinstance(readability, dict) else 0
        if isinstance(readability, dict) and readability.get("ok") and readability_text and readability_length >= 80:
            ...
            summary_text = (
                str(readability.get("excerpt", "")).strip()
                or " ".join(readability_text.split())[:500]
            )
            headings = [
                {
                    "ref_id": "",
                    "text": str(readability.get("title") or snapshot.title or "").strip()[:140],
                    "tag": "title",
                }
            ] if (readability.get("title") or snapshot.title) else []
            return json.dumps(
                {
                    "url": snapshot.url,
                    "title": str(readability.get("title") or snapshot.title or ""),
                    "page_type": "content_page",
                    "summary": summary_text,
                    "summary_strategy": "readability",
                    "content": readability_text,
                    "byline": str(readability.get("byline", "")).strip(),
                    "site_name": str(readability.get("site_name", "")).strip(),
                    "lang": str(readability.get("lang", "")).strip(),
                    "headings": headings,
                    "key_targets": [],
                    ...
                    "degraded_mode": False,
                    "degraded_reason": "",
                    "content_length": readability_length,
                },
                ensure_ascii=False,
            )
