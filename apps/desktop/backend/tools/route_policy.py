"""
ADELE - deterministic web route policy.
"""
...
def decide_web_route(
    *,
    ...
    if target_type == "search_results":
        return {
            "route": "background_fetch",
            "reason": "Plain search-result research uses background search by default.",
            "confidence": 0.93,
            "policy": "deterministic-web-policy",
        }
    ...
    is_serp = _is_search_results_url(current_url) if current_url else False
    if (
