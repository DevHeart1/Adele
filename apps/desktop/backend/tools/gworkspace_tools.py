...
2. **Google REST API** (optional speed enhancement) — if the user has set
   up an OAuth2 token at ``<ADELE_DATA_DIR>/gcloud_token.json`` the tool uses
...
def _token_path() -> str:
    from runtime_paths import get_adele_data_root

    return os.path.join(get_adele_data_root(), "gcloud_token.json")


_VISION_COORD_RE = re.compile(r"\(?\b(?:x\s*[:=]\s*)?(\d{1,4})\s*,\s*(?:y\s*[:=]\s*)?(\d{1,4})\b\)?", re.I)
_LAST_VISUAL_COORDS: tuple[int, int] | None = None
...
async def _gdocs_read_body(allow_vision: bool = True) -> str:
...
async def _gdocs_replace_body(body: str) -> bool:
    ...
    for attempt, focus_fn in enumerate([
        ...
        readback = await _gdocs_read_body(allow_vision=False)
...
async def _vision_locate_and_click(description: str, hint: str = "") -> bool:
    ...
    try:
        ...
        print(f"[_vision_locate_and_click] ⚠ Fast locate error: {e}")

    if not coords:
        coords = _LAST_VISUAL_COORDS
...
async def _vision_verify_content(expected_snippet: str) -> bool:
    ...
    try:
        ...
        global _LAST_VISUAL_COORDS
        coords = _extract_visual_coordinates(str(result or ""))
        if coords:
            _LAST_VISUAL_COORDS = coords
...
async def gdocs_read(doc_url_or_id: str) -> str:
    ...
    if text and len(_normalize_doc_text(text)) >= 5:
        return json.dumps({"ok": True, "doc_id": doc_id,
                           ...
                           "note": "Read via browser automation (DOM/clipboard/vision). "
                                   "For full fidelity set up OAuth in <ADELE_DATA_DIR>/gcloud_token.json."},
                          ensure_ascii=False)
    ...
    return json.dumps({"ok": False, "doc_id": doc_id,
                       ...
                       "note": "For full fidelity and reliability without the browser extension, "
                               "set up OAuth in <ADELE_DATA_DIR>/gcloud_token.json."},
                      ensure_ascii=False)
