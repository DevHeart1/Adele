...
def _screenshot_work_dir() -> str:
    try:
        ...
        return os.path.join(tempfile.gettempdir(), "adele")
...
async def open_app(app_name: str) -> str:
    if os.name == "nt":
        from windows_desktop import open_app as windows_open_app

        return await windows_open_app(app_name, KNOWN_URLS)
...
async def close_window() -> str:
    if os.name == "nt":
        from windows_desktop import close_window as windows_close_window

        return await windows_close_window()
...
async def quit_app(app_name: str) -> str:
    if os.name == "nt":
        ...
        try:
            ...
            return f"ERROR quitting {target}: {str(exc)[:200]}"
...
async def run_shortcut(keys: str) -> str:
    if os.name == "nt":
        try:
            from windows_desktop import run_shortcut as windows_run_shortcut

            await windows_run_shortcut(keys)
            return f"Pressed shortcut '{keys}'."
        except Exception as exc:
            return f"ERROR running Windows shortcut '{keys}': {str(exc)[:200]}"
...
async def read_screen(question: str = "") -> str:
    ...
    try:
        ...
        response = client.models.generate_content(
            model=os.environ.get("GEMINI_POWERFUL_MODEL", "gemini-2.5-pro"),
            contents=[
                types.Content(
                    role="user",
                    parts=[
...
async def clipboard_ops(action: str, text: str = "", image_path: str = "") -> str:
    try:
        if os.name == "nt":
            from windows_desktop import get_clipboard_text, paste_text, run_shortcut, set_clipboard_text

            if action == "get":
                content = await get_clipboard_text()
                if content:
            ...
            if action == "set_image":
                return (
                    "ERROR: Windows image clipboard support is not wired yet. "
                    "Use capture_region_screenshot/save_image to create the file, then insert it through the target app or browser."
                )
            if action == "get_image_info":
                return "Windows image clipboard inspection is not wired yet."
...
async def get_ui_tree(app_name: str = "", search_term: str = "") -> str:
    if os.name == "nt":
        from windows_desktop import get_ui_tree as windows_get_ui_tree

        return await windows_get_ui_tree(app_name, search_term)
...
async def press_key(key: str, times: int = 1) -> str:
    ...
    if os.name == "nt":
        try:
            from windows_desktop import press_key as windows_press_key

            await windows_press_key(key, times)
            return f"Pressed '{key}' {times} time(s)"
        except Exception as exc:
            return f"ERROR pressing Windows key '{key}': {str(exc)[:200]}"
...
async def click_ui(description: str, app_name: str = "", click_type: str = "single") -> str:
    ...
    try:
        ...
        if not match:
            # 3) OCR/screenshot fallback — find the element on screen using Gemini fast tier
...
async def type_in_field(field_description: str, text: str, app_name: str = "", clear_first: bool = False) -> str:
    ...
    try:
        ...
        if match:
            ...
            # 3) OCR/screenshot fallback — find the field using Gemini fast tier
...
async def _run_browser_js(js: str, app_name: str = "") -> str:
    ...
    if os.name == "nt":
        return (
            "ERROR: legacy browser JavaScript execution is macOS-only. "
            "Use the Browser Bridge tools (browser_snapshot, browser_find, browser_click_ref, browser_type_ref) on Windows."
        )
...
async def save_image(url: str, filename: str = "") -> str:
    ...
    try:
        req = urllib.request.Request(url, headers={
            "User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7) ADELE/1.0"
        })
...
async def capture_region_screenshot(x: int, y: int, width: int, height: int) -> str:
    ...
    try:
        if os.name == "nt":
            from windows_desktop import capture_screenshot as windows_capture_screenshot

            await windows_capture_screenshot(filepath, (x, y, width, height))
            if not os.path.exists(filepath):
            ...
            return json.dumps({
                ...
                "size_kb": round(size_kb, 1),
                "message": f"Captured {width}x{height} region at ({x},{y})."
            })
