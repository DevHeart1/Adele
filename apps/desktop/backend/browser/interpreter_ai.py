...
def _get_local_provider(model_name: str):
    mode = (os.environ.get("ADELE_LLM_PROVIDER") or "").strip().lower()
    base_url = (
...
async def _generate_with_flash(*, system_prompt: str, user_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    model_name = os.environ.get("GEMINI_FAST_MODEL", DEFAULT_GEMINI_MODEL).strip() or DEFAULT_GEMINI_MODEL
...
async def summarize_page_with_flash(snapshot) -> Dict[str, Any]:
    ...
    system_prompt = (
        "You are ADELE's browser page interpreter. Analyze the provided browser snapshot and return ONLY JSON. "
...
async def summarize_scraped_page_with_flash(
    *,
    ...
    system_prompt = (
        "You are ADELE's background web page summarizer. Analyze the provided scraped page text and return ONLY JSON. "
...
async def extract_structured_items_with_flash(
    ...
    *,
    ...
    system_prompt = (
        "You are ADELE's browser structured-data extractor. Analyze the provided browser snapshot and return ONLY JSON. "
...
async def decide_web_route_with_flash(
    *,
    ...
    system_prompt = (
        "You are ADELE's web gateway route planner. Choose the best route for a web information request. "
...
async def choose_search_result_with_flash(
    *,
    ...
    system_prompt = (
        "You are ADELE's browser search-result chooser. Choose the single best result to follow next. "
