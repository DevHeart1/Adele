...
class BrowserResolver:
    ...
    async def resolve_via_cdp(
        self,
        query: str,
        action: str = "click",
    ) -> Optional[ElementRef]:
        """Resolves target element coordinates directly in the browser via CDP."""
        from .cdp import cdp_client
        match = await cdp_client.resolve_element_via_cdp(query)
        if not match:
            return None

        x = int(match["screenX"] + match["width"] / 2)
        y = int(match["screenY"] + match["height"] / 2)
