...
# Windows consoles often default to cp1252; Unicode log glyphs must not crash startup.
...
from runtime_paths import ensure_adele_data_layout

_data_root = ensure_adele_data_layout()
print(f"[Local] ADELE local data directory: {_data_root}")
...
# ADELE Agent (V2 runtime)
from agent import create_agent
import agent.perception as perception
from agent.local_action_contract import get_local_action_contract
from runtime_state import runtime_state_store
from voice.tts import get_tts_engine, prepare_for_speech, tts_backend
from voice.transcription import stt_backend, transcribe_wav_bytes
...
AGENT_VERSION = os.environ.get("ADELE_AGENT_VERSION", "v2")
...
class VoiceAssistant:
    def __init__(self):
        ...
        stt = stt_backend()
        if stt == "elevenlabs":
            model = (os.environ.get("ADELE_STT_MODEL_ID") or "scribe_v2").strip()
            print(f"[Voice] STT: ElevenLabs Scribe ({model})")
        else:
            print("[Voice] STT: Google Web Speech")

        tts = tts_backend()
        if tts == "elevenlabs":
            voice = (os.environ.get("ELEVENLABS_VOICE_ID") or "").strip() or "(default)"
            model = (os.environ.get("ELEVENLABS_TTS_MODEL") or "eleven_v3").strip()
            print(f"[Voice] TTS: ElevenLabs (voice={voice}, model={model})")
        else:
            print("[Voice] TTS: Google Cloud Neural2")
        ...
        try:
            if pvporcupine is None:
                ...
                custom_ppn = os.path.join(project_root, "hey_adele.ppn")
                ...
                if ppn_path:
                    ...
                    print(f"[Voice] OK Porcupine initialized with 'Hey ADELE' from {ppn_path}")
                else:
                    self.porcupine = pvporcupine.create(
                        access_key=PICOVOICE_ACCESS_KEY,
                        keywords=["porcupine"]
                    )
                    print("[Voice] WARN hey_adele.ppn not found — using built-in 'Porcupine' keyword")
                    print(f"[Voice]   Searched: {custom_ppn}")
                    print(f"[Voice]   Searched: {servers_ppn}")
        except Exception as e:
            print(f"[Voice] ERR Porcupine init failed: {e}")
        ...
        # Push-to-talk: Alt hold streams mic until release (Scribe v2 STT)
    ...
    async def run_agent_text(self, websocket, text: str):
        ...
        try:
            if _is_cursor_demo_request(text):
            ...
            elif text.strip().rstrip("?.!").lower() == "what am i looking at":
                # Intercept the onboarding test query
                await ws_callback({
                    "type": "tool_event",
                    "phase": "start",
                    "actionId": f"{task_id}:read-screen-test",
                    "tool": "read_screen",
                })
                await ws_callback({
                    "type": "doing",
                ...
                description = (
                    "I see you are looking at the ADELE onboarding wizard. "
                ...
                # Move cursor to show pointer works
                await ws_callback({
                    "type": "doing",
                    "text": "Testing pointer overlay...",
                    "tool": "show_cursor",
                    "variant": "",
                })
                await ws_callback({
    ...
    async def start_agent_text_task(self, websocket, text: str, source: str = "text_input"):
        ...
        async def runner():
            try:
                ...
                await websocket.send(json.dumps({
                    "type": "response",
                    "payload": {"text": "ADELE took too long to respond, so I stopped that task.", "app": ""}
                }))
                ...
                await websocket.send(json.dumps({
                    "type": "response",
                    "payload": {"text": "ADELE hit an internal error while processing that request.", "app": ""}
                }))
                await websocket.send(json.dumps({"type": "status", "state": "state-idle"}))
    ...
    async def handle_audio_chunk(self, websocket, b64_payload):
        ...
        try:
            ...
            if self.ptt_active:
                await self.buffer_ptt_audio(websocket, pcm_data)
    ...
    async def start_push_to_talk(self, websocket):
        """Alt-hold: record immediately; transcribe on release."""
        if self.state in ("LOADING", "DOING"):
            return
        print("[Voice] Push-to-talk start (Alt held)")
    ...
    async def stop_push_to_talk(self, websocket):
        """Alt released: run Scribe on buffered audio."""
        if not self.ptt_active:
            return
        self.ptt_active = False
        buffer_len = len(self.audio_buffer)
        print(f"[Voice] Push-to-talk stop — transcribing {buffer_len} bytes")
        if buffer_len < self.MIN_BUFFER_SIZE:
            self.state = "IDLE"
            self.audio_buffer = bytearray()
            await websocket.send(json.dumps({"type": "status", "state": "state-idle"}))
            return
        self.state = "LOADING"
        await websocket.send(json.dumps({"type": "progress", "state": "state-loading"}))
        asyncio.create_task(self.transcribe_and_act(websocket))

    async def buffer_ptt_audio(self, websocket, pcm_data):
        """Append audio while Alt is held; no silence detection."""
        ...
        async for message in websocket:
            try:
                ...
                if msg_type == "audio_chunk":
                    ...
                    await assistant.start_push_to_talk(websocket)
                ...
                elif msg_type == "agent_contract_observe":
                    contract = get_local_action_contract()
                    payload = data.get("payload") or {}
                    result = await contract.observe(
                        trace_id=str(payload.get("trace_id") or data.get("trace_id") or ""),
                        include_vision=bool(payload.get("include_vision", False)),
                    )
                    await websocket.send(json.dumps({
                        "type": "agent_contract_observe_result",
                        "request_id": data.get("request_id") or payload.get("request_id") or "",
                        "payload": result,
                    }))
                elif msg_type == "agent_contract_visual_context":
                    contract = get_local_action_contract()
                    payload = data.get("payload") or {}
                    result = await contract.visual_context(
                        trace_id=str(payload.get("trace_id") or data.get("trace_id") or ""),
                        intent=str(payload.get("intent") or ""),
                        include_screenshot=bool(payload.get("include_screenshot", True)),
                    )
                    await websocket.send(json.dumps({
                        "type": "agent_contract_visual_context_result",
                        "request_id": data.get("request_id") or payload.get("request_id") or "",
                        "payload": result,
                    }))
                elif msg_type == "agent_contract_ground_target":
                    contract = get_local_action_contract()
                    payload = data.get("payload") or {}
                    result = await contract.ground_target(
                        trace_id=str(payload.get("trace_id") or data.get("trace_id") or ""),
                    ...
                    await websocket.send(json.dumps({
                        "type": "agent_contract_ground_target_result",
                        "request_id": data.get("request_id") or payload.get("request_id") or "",
                        "payload": result,
                    }))
                elif msg_type == "agent_contract_act":
                    contract = get_local_action_contract()
                    payload = data.get("payload") or {}
                    if data.get("trace_id") and not payload.get("trace_id"):
                        payload["trace_id"] = data.get("trace_id")
                    result = await contract.act(payload)
                    await websocket.send(json.dumps({
                        "type": "agent_contract_act_result",
                        "request_id": data.get("request_id") or payload.get("request_id") or "",
                        "payload": result,
                    }))
                elif msg_type == "agent_contract_history":
                    contract = get_local_action_contract()
                    payload = data.get("payload") or {}
                    result = contract.history(str(payload.get("trace_id") or data.get("trace_id") or ""))
                    await websocket.send(json.dumps({
                        "type": "agent_contract_history_result",
                        "request_id": data.get("request_id") or payload.get("request_id") or "",
                        "payload": result,
                    }))
                elif msg_type == "list_pending_reviews":
                    from agent.memory import get_mongo_db
                    db = get_mongo_db()
                    reviews = []
                    if db is not None:
                        try:
                            ...
                            cursor = db.memory_reviews.find({"status": "pending_review"})
                            for review in cursor:
                                r_id = str(review.get("_id"))
                                mem_id = review.get("memory_id")
                                # Fetch the associated memory
                                from bson import ObjectId
                                mem_doc = db.memories.find_one({"_id": ObjectId(mem_id)})
                                if mem_doc:
                                    reviews.append({
                                        "id": r_id,
                                        "memory_id": mem_id,
                                        "title": mem_doc.get("title", ""),
                                        "content": mem_doc.get("content", ""),
                                        "category": mem_doc.get("category", "notes"),
                                        "tags": mem_doc.get("tags", []),
                                        "reason_for_saving": review.get("reason_for_saving", ""),
                                        "created_at": mem_doc.get("created_at", review.get("updated_at", 0)),
                                    })
                        except Exception as e:
                            print(f"[WebSocket] list_pending_reviews failed: {e}")
                    await websocket.send(json.dumps({
                        "type": "pending_reviews_list",
                        "reviews": reviews
                    }))
                elif msg_type == "approve_memory":
                    from agent.memory import get_mongo_db
                    db = get_mongo_db()
                    success = False
                    memory_id = data.get("memory_id")
                    if db is not None and memory_id:
                        try:
                            from bson import ObjectId
                            # Update memory status
                            db.memories.update_one(
                                {"_id": ObjectId(memory_id)},
                                {"$set": {"review_status": "approved", "updated_at": time.time()}}
                            )
                            # Remove review doc
                            db.memory_reviews.delete_one({"memory_id": memory_id})
                            success = True
                            print(f"[WebSocket] Approved memory candidate: {memory_id}")
                        except Exception as e:
                            print(f"[WebSocket] approve_memory failed: {e}")
                    await websocket.send(json.dumps({
                        "type": "approve_memory_result",
                        "memory_id": memory_id,
                        "success": success
                    }))
                elif msg_type == "reject_memory":
                    from agent.memory import get_mongo_db
                    db = get_mongo_db()
                    success = False
                    memory_id = data.get("memory_id")
                    if db is not None and memory_id:
                        try:
                            ...
                            db.memories.update_one(
                                {"_id": ObjectId(memory_id)},
                                {"$set": {"review_status": "rejected", "updated_at": time.time()}}
                            )
                            # Remove review doc
                            db.memory_reviews.delete_one({"memory_id": memory_id})
                            success = True
                            print(f"[WebSocket] Rejected memory candidate: {memory_id}")
                        except Exception as e:
                            print(f"[WebSocket] reject_memory failed: {e}")
                    await websocket.send(json.dumps({
                        "type": "reject_memory_result",
                        "memory_id": memory_id,
                        "success": success
                    }))
                elif msg_type == "edit_memory":
                    from agent.memory import get_mongo_db
                    db = get_mongo_db()
                    success = False
                    memory_id = data.get("memory_id")
                    ...
                    if db is not None and memory_id:
                        try:
                            ...
                            if category is not None:
                                update_fields["category"] = category.strip().lower()
                            ...
                            if update_fields:
                                ...
                                # Re-fetch full doc to compute new embedding
                                doc = db.memories.find_one({"_id": ObjectId(memory_id)})
                                if doc:
                                    ...
                                    # Re-compute vector embedding
                                    try:
                                        from agent.rag import get_rag_engine
                                        rag = get_rag_engine()
                                        text_chunk = f"{doc.get('title', '')}\n{doc.get('content', '')}"
                                        vector = rag.embed(text_chunk)
                                        if vector:
                                        ...
                                        print(f"[WebSocket] Failed to re-compute embedding on edit: {emb_err}")
                                        
                                success = True
                                print(f"[WebSocket] Edited memory candidate: {memory_id}")
                        except Exception as e:
                            print(f"[WebSocket] edit_memory failed: {e}")
                    await websocket.send(json.dumps({
                        "type": "edit_memory_result",
                        "memory_id": memory_id,
                        "success": success
                    }))
