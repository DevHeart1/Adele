...
from runtime_paths import ensure_adele_data_layout, get_adele_data_root
...
try:
    ...
    pymongo = None
    ObjectId = None
    _PymongoMongoClient = None
    PYMONGO_AVAILABLE = False


def _adele_fast_mongo_kwargs(kwargs: dict) -> dict:
    """Keep a broken Atlas URI from stalling every ADELE startup/request."""
    next_kwargs = dict(kwargs or {})
    for key in ("serverSelectionTimeoutMS", "connectTimeoutMS", "socketTimeoutMS"):
        try:
            next_kwargs[key] = min(int(next_kwargs.get(key, 3000)), 3000)
        except (TypeError, ValueError):
            next_kwargs[key] = 3000
    return next_kwargs


def MongoClient(*args, **kwargs):
    uri = str(args[0] if args else kwargs.get("host", "") or "")
    if "mock_connection" in uri:
        try:
            import mongomock
            ...
            pass
    client_factory = getattr(pymongo, "MongoClient", None) if pymongo is not None else _PymongoMongoClient
    return client_factory(*args, **_adele_fast_mongo_kwargs(kwargs))

MONGODB_URI = os.environ.get("ADELE_MONGODB_URI") or os.environ.get("MONGODB_URI")
MONGODB_DB = os.environ.get("ADELE_MONGODB_DB") or os.environ.get("MONGODB_DB") or "adele"
...
def _load_mongo_config_from_disk() -> tuple[str | None, str]:
    """Read MongoDB settings written by Electron (credentials are encrypted there)."""
    uri = None
    db_name = MONGODB_DB
    try:
        config_path = os.path.join(get_adele_data_root(), "mongodb.json")
        if os.path.exists(config_path):
            with open(config_path, "r", encoding="utf-8") as f:
                config = json.load(f)
            uri = (config.get("mongodb_uri") or "").strip() or None
            db_name = (config.get("mongodb_db") or db_name).strip() or db_name
    except Exception:
        pass
    return uri, db_name

def get_mongo_db():
    global _mongo_client, _mongo_db, _mongo_unavailable
    if not PYMONGO_AVAILABLE:
        return None
    if _mongo_db is not None:
        return _mongo_db

    uri = os.environ.get("ADELE_MONGODB_URI") or os.environ.get("MONGODB_URI") or MONGODB_URI
    db_name = os.environ.get("ADELE_MONGODB_DB") or os.environ.get("MONGODB_DB") or MONGODB_DB
    if _mongo_unavailable and not uri:
        return None
    
    if not uri:
        file_uri, file_db = _load_mongo_config_from_disk()
        if file_uri:
            uri = file_uri
            db_name = file_db
    
    if not uri:
        # Legacy fallback: preferences.json
        try:
            pref_path = os.path.join(get_adele_data_root(), "preferences.json")
            if os.path.exists(pref_path):
                with open(pref_path, "r") as f:
                    prefs = json.load(f)
                    uri = prefs.get("mongodb_uri")
                    db_name = prefs.get("mongodb_db") or db_name
        except Exception:
            pass
    ...
    try:
        _mongo_client = MongoClient(uri, serverSelectionTimeoutMS=MONGODB_CONNECT_TIMEOUT_MS)
        # Ping the connection
        _mongo_client.admin.command('ping')
        _mongo_db = _mongo_client[db_name]
        print(f"[MongoDB] Connected to database: {db_name}")
        _initialize_mongo_collections(_mongo_db)
        return _mongo_db
    except Exception as e:
        _mongo_unavailable = True
        _mongo_client = None
        _mongo_db = None
        print(f"[MongoDB] Connection failed: {e}. Falling back to local files for this session.")
        return None

def _initialize_mongo_collections(db):
    try:
        collections = [
            "users", "sessions", "screen_events", "memories", 
            "actions", "skills", "mistakes", "documents", 
            "embeddings", "memory_reviews",
            # Expanded collections
            "projects", "memory_edges", "workflows", "app_profiles", 
        ...
        for col in collections:
            if col not in existing:
                ...
                print(f"[MongoDB] Created collection: {col}")
                
        # Create indexes
        db.memories.create_index([("title", pymongo.TEXT), ("content", pymongo.TEXT)])
        db.memories.create_index([("category", pymongo.ASCENDING)])
        db.memories.create_index([("review_status", pymongo.ASCENDING)])
        db.memories.create_index([("type", pymongo.ASCENDING)])
        db.memories.create_index([("created_at", pymongo.DESCENDING)])
        
        db.embeddings.create_index([("memory_id", pymongo.ASCENDING)])
        
        db.screen_events.create_index([("timestamp", pymongo.DESCENDING)])
        db.screen_events.create_index([("source_session", pymongo.ASCENDING)])
        
        db.sessions.create_index([("updated_at", pymongo.DESCENDING)])
        
        db.actions.create_index([("timestamp", pymongo.DESCENDING)])
        
        db.memory_reviews.create_index([("memory_id", pymongo.ASCENDING)])
        db.memory_reviews.create_index([("status", pymongo.ASCENDING)])
        
        print("[MongoDB] Database collections and indices initialized.")
    except Exception as e:
        print(f"[MongoDB] Failed to initialize collections/indices: {e}")


def _adele_root() -> str:
    return get_adele_data_root()


def _sessions_dir() -> str:
    return os.path.join(_adele_root(), "sessions")


def _vault_dir() -> str:
    return os.path.join(_adele_root(), "vault")
...
class ConversationMemory:
    ...
    def _try_resume_session(self, resume_window: float = 1800.0):
        ...
        with self._io_lock:
            ...
            if db is not None:
                try:
                    doc = db.sessions.find_one(sort=[("updated_at", pymongo.DESCENDING)])
                    if doc:
                        updated_at = doc.get("updated_at", 0)
                        if time.time() - updated_at <= resume_window:
                            self._turns = doc.get("turns", [])
                            self._session_id = doc.get("session_id", str(doc.get("_id")))
                            self._session_summary = doc.get("summary", "")
                            self._last_activity = updated_at
                            print(f"[MongoDB] Resumed session: {self._session_id} (updated {time.time() - updated_at:.1f}s ago)")
                            return
                except Exception as e:
                    print(f"[MongoDB] Failed to resume session: {e}")
    ...
    def _flush_save(self):
        """Actually write the session to disk/MongoDB (called by debounce timer or explicitly)."""
        ...
        with self._io_lock:
            ...
            if db is not None:
                try:
                    ...
                    print(f"[MongoDB] Failed to save session: {e}")

            # Local file save (backup and fallback)
            try:
                _ensure_dir()
...
class WorkingMemory:
    ...
    *structured* layer that feeds compact, high-signal context into the
    ...
    def log_action(self, tool: str, args: dict, result: str, success: bool = True, session_id: str = "unknown"):
        ...
        if db is not None:
            try:
                ...
                print(f"[MongoDB] Failed to log action: {e}")
...
class UserPreferences:
    ...
    def __init__(self):
        _ensure_dir()
        self._path = os.path.join(_adele_root(), "preferences.json")
    ...
    def _load(self) -> dict:
        ...
        if db is not None:
            try:
                user_doc = db.users.find_one({"_id": "default_user"})
                if user_doc and "preferences" in user_doc:
                    return user_doc["preferences"]
            except Exception as e:
                print(f"[MongoDB] Load preferences failed: {e}")
        # Local fallback
    ...
    def _save(self):
        ...
        if db is not None:
            try:
                db.users.update_one(
                    {"_id": "default_user"},
                    {"$set": {"preferences": self._data, "updated_at": time.time()}},
                    upsert=True
                )
                return
            except Exception as e:
                print(f"[MongoDB] Save preferences failed: {e}")
        # Local fallback
...
class UserProfile:
    ...
    def __init__(self):
        _ensure_dir()
        self._path = os.path.join(_adele_root(), "user_profile.json")
    ...
    def _load(self) -> dict:
        ...
        if db is not None:
            try:
                ...
                print(f"[MongoDB] Load profile failed: {e}")
        # Local fallback
        if os.path.exists(self._path):
            try:
                with open(self._path, "r") as f:
    ...
    def _save(self):
        ...
        if db is not None:
            try:
                db.users.update_one(
                    {"_id": "default_user"},
                    {"$set": {"profile": self._profile, "updated_at": time.time()}},
                    upsert=True
                )
                return
            except Exception as e:
                print(f"[MongoDB] Save profile failed: {e}")
        # Local fallback
        try:
            with open(self._path, "w") as f:
...
class TaskStore:
    ...
    def __init__(self):
        _ensure_dir()
        self._path = os.path.join(_adele_root(), "tasks.json")
    ...
    def _load(self) -> dict[str, BackgroundTask]:
        ...
        if db is not None:
            try:
                ...
                print(f"[MongoDB] Load tasks failed: {e}")
        # Local fallback
        if os.path.exists(self._path):
            try:
                with open(self._path, "r") as f:
    ...
    def _save(self):
        ...
        if db is not None:
            try:
                ...
                print(f"[MongoDB] Save tasks failed: {e}")
        # Local fallback
        data = {}
        for tid, task in self._tasks.items():
            data[tid] = {
...
class VaultMemory:
    ...
    JSON files under ``<ADELE_DATA_DIR>/vault/<category>/``.  Supports TF-IDF search
    ...
    def store(
        ...
        *,
        ...
        if prefs.get("memory_disable_sensitive_apps", True):
            source_lower = (source or "").lower()
            sensitive_terms = (
                "1password", "bitwarden", "lastpass", "keepass", "dashlane",
                "password manager", "credential", "bank", "paypal", "venmo",
            )
            if any(term in source_lower for term in sensitive_terms):
                return {
                    "ok": False,
                    "skipped": "sensitive_app",
                    "error": "Memory is disabled for sensitive apps.",
                }

        if prefs.get("memory_summaries_only", True) and category in {"screen_context", "screen_events"}:
            return {
                "ok": False,
                "skipped": "summaries_only",
                "error": "Only summaries are saved — raw screen memories are disabled.",
            }
        ...
        if db is not None:
            try:
                mem_type = category
                if mem_type not in {"preference", "task", "project", "mistake", "workflow", "screen_context", "decision", "document_fact"}:
                    mem_type = "document_fact"

                # Check for require_memory_approval setting in preferences
                review_status = "pending_review"
                if not prefs.get("require_memory_approval", True):
                    review_status = "approved"

                # Check for near-duplicate: same category + very similar title
                existing = db.memories.find_one({"category": category, "title": title})
                ...
                if existing:
                    db.memories.update_one({"_id": existing["_id"]}, {"$set": doc})
                    entry_id = str(existing["_id"])
                    action = "updated"
                else:
                    doc["created_at"] = time.time()
                    entry_id = str(db.memories.insert_one(doc).inserted_id)
                    action = "created"
                ...
                try:
                    ...
                    print(f"[MongoDB] Failed to store embedding: {e}")

                # If pending review, create memory_reviews record
                if review_status == "pending_review":
                    db.memory_reviews.update_one(
                        {"memory_id": entry_id},
                        {"$set": {
                            "memory_id": entry_id,
                            "content": content,
                            "reason_for_saving": "Auto-curated fact from conversation" if "curated" in source else "User requested remembering",
                ...
                print(f"[MongoDB] 💾 Stored vault entry: [{category}] {title[:60]} ({len(content)} chars)")
                return {"ok": True, "id": entry_id, "action": action}
            except Exception as e:
                print(f"[MongoDB] Failed to store memory: {e}. Falling back to local files.")
    ...
    def recall(
        ...
        """Search the vault by query, category, and/or tags.
        ...
        if db is not None:
            try:
                # Retrieve only approved memories by default
                filter_doc = {"review_status": "approved"}
                if category:
                    filter_doc["category"] = category.strip().lower()
                if tags:
                    tag_list = [t.strip().lower() for t in tags if t.strip()]
                    filter_doc["tags"] = {"$in": tag_list}

                candidates = list(db.memories.find(filter_doc))
                ...
                for doc in candidates:
                    doc["id"] = str(doc["_id"])
                    doc["content_preview"] = doc.get("content", "")[:200]
                    ...
                    try:
                        ...
                        if query_vector:
                            ...
                            for item in candidates:
                                emb_doc = db.embeddings.find_one({"memory_id": item["id"]})
                                if emb_doc and emb_doc.get("vector"):
                                    sim = rag.cosine_similarity(query_vector, emb_doc["vector"])
                                    scored.append((item, sim))
                                else:
                                    scored.append((item, 0.0))
                            scored.sort(key=lambda x: x[1], reverse=True)
                            return [x[0] for x in scored[:max_results]]
                    except Exception as e:
                        print(f"[MongoDB] Vector similarity search failed: {e}. Falling back to TF-IDF.")
                ...
                print(f"[MongoDB] Recall failed: {e}. Falling back to local files.")
    ...
    def delete(self, entry_id: str) -> bool:
        ...
        if db is not None:
            try:
                res = db.memories.delete_one({"_id": ObjectId(entry_id)})
                db.embeddings.delete_one({"memory_id": entry_id})
                db.memory_reviews.delete_one({"memory_id": entry_id})
                print(f"[MongoDB] 🗑 Deleted memory: {entry_id}")
                return res.deleted_count > 0
            except Exception as e:
                print(f"[MongoDB] Delete failed: {e}. Falling back to local files.")
    ...
    def list_entries(self, category: str = "", limit: int = 50) -> list[dict]:
        ...
        if db is not None:
            try:
                ...
                if category:
                    filter_doc["category"] = category.strip().lower()
                cursor = db.memories.find(filter_doc).sort("updated_at", -1).limit(limit)
                entries = []
                for doc in cursor:
                    doc["id"] = str(doc["_id"])
                    doc["content_preview"] = doc.get("content", "")[:200]
                ...
                print(f"[MongoDB] List entries failed: {e}. Falling back to local files.")
    ...
    def get_stats(self) -> dict:
        ...
        if db is not None:
            try:
                total = db.memories.count_documents({})
                by_cat = {}
                pipeline = [{"$group": {"_id": "$category", "count": {"$sum": 1}}}]
                for group in db.memories.aggregate(pipeline):
                    cat = group["_id"] or "notes"
                    by_cat[cat] = group["count"]
                return {"total_entries": total, "by_category": by_cat}
            except Exception as e:
                print(f"[MongoDB] Get stats failed: {e}. Falling back to local files.")
