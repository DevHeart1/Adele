...
from tools.selector import expand_milestone_hint_tools, resolve_milestone_tool_tiers
...
You are the execution engine for ADELE, a cross-platform desktop AI assistant.
...
class MilestoneExecutor:
    ...
    async def _provider_generate_with_compat(self, **kwargs):
        try:
            return await self.provider.generate(**kwargs)
        except TypeError as exc:
            optional_keys = ("thinking_level", "enable_builtin_tools")
            trimmed = dict(kwargs)
            changed = False
    ...
    def _resolve_allowed_tools(
        ...
        return resolve_milestone_tool_tiers(milestone.hint_tools or [], request_scope_tool_names)

    def _is_action_relevant_to_milestone(self, action: MilestoneAction, milestone: Milestone) -> bool:
        if not milestone.hint_tools:
            return True
    ...
    async def _stream_llm_decision(
        ...
        try:
            response = await self._provider_generate_with_compat(
                messages=[{"role": "user", "parts": [{"text": prompt}]}],
                system_prompt=MILESTONE_EXECUTOR_SYSTEM,
                tools=[],
                temperature=0.15,
    ...
    async def execute_milestone(
        ...
        while action_num < _HARD_SAFETY_CAP:
            ...
            if priority_tool_names is not None and tool not in priority_tool_names:
                ...
                if milestone.hint_tools:
                    consecutive_rejections += 1
                    decision_failures += 1
                    last_result = (
                        f"Tool '{tool}' is outside this milestone's suggested evidence-gathering tools. "
        ...
        if len(actions) >= 2:
            ...
            if last.success:
                ...
                for action in reversed(actions):
                    ...
                    if action.success and sig == last_sig:
                        identical_streak += 1
                    else:
                        break
    ...
    def _is_substantive_result(self, action: MilestoneAction) -> bool:
        ...
        if not action.success:
            return False
        if action.tool in _TRIVIAL_PROGRESS_TOOLS or action.tool in _LOW_SIGNAL_ACTION_TOOLS:
            return False
    ...
    def _completion_has_evidence(
        ...
        if not actions:
            ...
            if deliverable_text and not evidence_sensitive:
                return True, ""
        ...
        if any(
            action.success
            and action.tool in evidence_tools
            and self._is_action_relevant_to_milestone(action, milestone)
            and len((action.result or "").strip()) > 60
            and not self._is_known_empty_json(action.result or "")
            for action in actions
        ):
            return True, ""
        if "result" in deliverable_text.lower() and any(
            action.success
            and action.tool == "web_search"
            and self._is_action_relevant_to_milestone(action, milestone)
            and len((action.result or "").strip()) >= 10
            for action in actions
        ):
            return True, ""
        ...
        successful_nontrivial = [
            ...
            if (
                ...
                and action.tool not in _LOW_SIGNAL_ACTION_TOOLS
