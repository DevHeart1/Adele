"""
Focused reliability checks for the ADELE recovery plan.
...
from agent.core_v2 import AdeleAgentV2, PendingPlanState
...
def test_runtime_state_prefers_bridge_browser_truth():
    ...
    browser_bridge.register_connection("bridge-session", "adele-browser-bridge")
...
def test_plan_tool_contract_requires_supported_hints():
    agent = AdeleAgentV2(use_planning=False, persist=False)
    plan = MilestonePlan(
...
def test_pending_plan_staleness_rules():
    ...
    stale, reason = agent._is_pending_plan_stale(
        web_pending,
        ContextSnapshot(active_app="Electron", browser_url=None, window_title="ADELE"),
    )
