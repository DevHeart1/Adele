...
AGENT_VERSION = os.environ.get("ADELE_AGENT_VERSION", "v1")
...
def create_temp_workspace():
    ...
    tmp = tempfile.mkdtemp(prefix="adele_bench_")
...
def print_final_report(suite_results: List[SuiteResult]):
    ...
    print(f"\n{'═'*70}")
    print(f"  📊  ADELE AGENT BENCHMARK REPORT")
    print(f"{'═'*70}\n")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Run ADELE Agent Benchmarks")
    parser.add_argument(
        "--agent", "-a",
        choices=["v1", "v2"],
